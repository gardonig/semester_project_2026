...
def apply_constraints_gt_free(
    ...
    for axis_name, matrix in [('vertical', poset.matrix_vertical)]:
        ...
        # Cache CC labeling and LCC extents — reused across constraint pairs
        cc_cache:  Dict[str, Tuple[np.ndarray, int, int]] = {}  # name → (labeled, n, lcc_label)
        lcc_extent_cache: Dict[str, Optional[Tuple[int, int]]] = {}  # name → (min, max)

        def get_cc(name: str):
            if name not in cc_cache:
                labeled, n = cc_label(constrained[name])
                if n == 0:
                    cc_cache[name] = (labeled, 0, 0)
                else:
                    sizes = np.bincount(labeled.ravel())
                    sizes[0] = 0
                    cc_cache[name] = (labeled, n, int(sizes.argmax()))
            return cc_cache[name]

        def get_lcc_extent(name: str):
            if name not in lcc_extent_cache:
                lcc_extent_cache[name] = axis_extent(
                    largest_connected_component(constrained[name]), vox_ax
                )
            return lcc_extent_cache[name]
        ...
        for i, si in enumerate(poset.structures):
            for j, sj in enumerate(poset.structures):
                ...
                if not constrained[name_i].any() or not constrained[name_j].any():
                    continue

                ext_j = get_lcc_extent(name_j)
                if ext_j is None:
                    continue
                min_j, max_j = ext_j

                labeled_i, n_i, lcc_label_i = get_cc(name_i)
                if n_i == 0:
                    continue
                ...
                # Invalidate caches for name_i if its mask changed
                if dirty:
                    cc_cache.pop(name_i, None)
                    lcc_extent_cache.pop(name_i, None)
...
def run_subject(
    ...
    # Only load structures that participate in at least one active constraint
    poset_names = {s.name for s in poset.structures}
    name_to_idx = {s.name: i for i, s in enumerate(poset.structures)}
    active = set()
    for matrix in [poset.matrix_vertical]:
        if matrix is None:
            continue
        for i, si in enumerate(poset.structures):
            for j, sj in enumerate(poset.structures):
    ...
    # Load only the needed predictions + get affine from first file
    predictions: Dict[str, np.ndarray] = {}
    affine = None
    header = None
    first_img = None
